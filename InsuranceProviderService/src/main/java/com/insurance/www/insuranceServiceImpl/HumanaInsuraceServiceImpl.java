...
@Service
@Slf4j
public class HumanaInsuraceServiceImpl implements HumanaInsuranceService {
	
	
		@Autowired
		private HttpServletResponse response;

		@Value("${humana.client.id}")
		private String clientId;

		@Value("${humana.client.secret}")
		private String clientSecret;

		@Value("${humana.auth-url}")
		private String humanaAuthUrl;

		@Value("${humana.callback-url}")
		...
		@Value("${humana.token-exchange-url}")
		private String humanaTokenExchangeUrl;

		@Value("${humana.patient-verification-url}")
		private String humanaPatientVerificationUrl;

		@Value("${humana.redirect-url}")
		private String redirectUri;

		@Value("${humana.error-url}")
		private String errorUri;

		public void authorize() throws IOException {
			String authorizationUrl = humanaAuthUrl + "?response_type=code" + "&client_id=" + clientId + "&redirect_uri="
					+ callBackUrl + "&scope=" + scopes + "&state=1234" + "&aud=" + aud;
			response.sendRedirect(authorizationUrl);
		}

		public void handleCallback(String authorizationCode) throws IOException {
			String tokenUrl = humanaTokenExchangeUrl;
			...
			if (tokenResponse.getStatusCode() == HttpStatus.OK) {
				...
				String patientApiUrl = humanaPatientVerificationUrl;
